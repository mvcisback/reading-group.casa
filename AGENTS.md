...
## High-level overview
- This is a lightweight FastAPI service that coordinates a reading group: register papers with optional reference links, vote once per session, and schedule upcoming titles with dates that can be updated or unselected.
...
## Code organization
- `main.py` hosts the FastAPI app, database helpers, and the POST handlers for `/papers` (register), `/papers/{id}/vote`, `/papers/{id}/select`, and `/papers/{id}/unselect`. The select handler now accepts a `selected_date` form value, while the unselect route clears `selected`/`selected_at`. `_hx_or_redirect()` chooses between an HTMX fragment and a redirect.
...
- `templates/partials/refresh.html` renders the scheduled readings list plus the candidate list. It is reused for both full-page renders and HTMX responses.
...
- The `selected` column stores `0/1` and the new `selected_at` column records the date a paper was scheduled. `_fetch_papers()` orders selected rows first with earlier dates ahead of later ones, `_selected_papers()` filters them for the upcoming panel, and the select form passes a `selected_date` value while `/papers/{id}/unselect` resets both columns.
...
- The `/papers/{id}/select` handler now accepts an optional `selected_date` (YYYY-MM-DD) from the date picker and uses today when omitted; invalid dates raise HTTP 400 while `/papers/{id}/unselect` clears the scheduling metadata.
